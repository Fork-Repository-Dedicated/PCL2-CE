...
namespace PCL
{
    public class MyTextBox : TextBox
    {
        ...
        public static readonly DependencyProperty CornerRadiusProperty = DependencyProperty.Register("CornerRadius", typeof(CornerRadius), typeof(MyTextBox), new PropertyMetadata(new CornerRadius(3d)));

        // 额外控件初始化

        private TextBlock _labWrong = null;
        ...
        private TextBlock _labHint = null;
        ...
        public List<RoutedEventHandler> ChangedEventList = new List<RoutedEventHandler>();
        ...
        void OnValidatedTextChanged(object sender, TextChangedEventArgs e)
        {
            foreach (RoutedEventHandler handler in ChangedEventList)
            {
                if (!(handler == null))
    ...
    /// </summary>
        public string ValidateResult
        {
            get
            {
                return Conversions.ToString(GetValue(ValidateResultProperty));
            }
            set
            {
        ...
        public static readonly DependencyProperty ValidateResultProperty = DependencyProperty.Register("ValidateResult", typeof(string), typeof(MyTextBox), new PropertyMetadata("", (MyTextBox d, DependencyPropertyChangedEventArgs e) => d.SetValue(IsValidatedPropertyKey, string.IsNullOrEmpty(Conversions.ToString(e.NewValue)))));
    ...
    /// </summary>
        public bool IsValidated
        {
            get
            {
                return Conversions.ToBoolean(GetValue(IsValidatedProperty));
            }
        }
        private static readonly DependencyPropertyKey IsValidatedPropertyKey = DependencyProperty.RegisterReadOnly("IsValidated", typeof(bool), typeof(MyTextBox), new PropertyMetadata(true));
    ...
    /// </summary>
        ...
        private System.Collections.ObjectModel.Collection<ValidateType> _ValidateRules = new System.Collections.ObjectModel.Collection<ValidateType>();
        /// <summary>
    /// 进行输入验证。
    /// </summary>
        ...
        private void ChangeValidateResult(bool IsSuccessful, bool IsLoaded)
        {
            if (IsLoaded && ModAnimation.AniControlEnabled == 0 && labWrong is not null)
            {
                if (IsSuccessful || !IsTextChanged)
                {
                    ...
                    ModAnimation.AniStart(new[] { ModAnimation.AaOpacity(labWrong, -labWrong.Opacity, 150), ModAnimation.AaHeight(labWrong, -labWrong.Height, 150, Ease: new ModAnimation.AniEaseOutFluent()), ModAnimation.AaCode(() => labWrong.Visibility = Visibility.Collapsed, After: true) }, "MyTextBox Validate " + Uuid);
                }
                else if (ShowValidateResult)
                {
                    ...
                    ModAnimation.AniStart(new[] { ModAnimation.AaOpacity(labWrong, 1d - labWrong.Opacity, 150), ModAnimation.AaHeight(labWrong, 21d - labWrong.Height, 150, Ease: new ModAnimation.AniEaseOutFluent()) }, "MyTextBox Validate " + Uuid);
                }
                else
                {
                    // 变为错误，但不显示文本
    ...
    /// </summary>
        ...
        public string HintText
        {
            get
            {
                return Conversions.ToString(GetValue(HintTextProperty));
            }
            set
            {
        ...
        public static readonly DependencyProperty HintTextProperty = DependencyProperty.Register("HintText", typeof(string), typeof(MyTextBox), new PropertyMetadata("", (t, e) => { if (t.labHint is not null) t.labHint.Text = string.IsNullOrEmpty(t.Text) ? t.HintText : ""; }));
        private void MyTextBox_TextChanged(MyTextBox sender, TextChangedEventArgs e)
        {
            ...
            {
                ModBase.Log(ex, "进行输入验证时出错", ModBase.LogLevel.Assert);
            }
        }
        ...
        private void RefreshColor()
        {
            try
            {
                ...
                {
                    // 有动画
                    ModAnimation.AniStart(new[] { ModAnimation.AaColor(this, BorderBrushProperty, ForeColorName, AnimationTime), ModAnimation.AaColor(this, BackgroundProperty, BackColorName, AnimationTime) }, "MyTextBox Color " + Uuid);
                }
                else
                {
                    // 无动画
        ...
        private void RefreshTextColor()
        {
            ...
            {
                // 有动画
                ModAnimation.AniStart(new[] { ModAnimation.AaColor(this, ForegroundProperty, IsEnabled ? "ColorBrushGray1" : "ColorBrushGray4", 200) }, "MyTextBox TextColor " + Uuid);
            }
            else
            {
                // 无动画
