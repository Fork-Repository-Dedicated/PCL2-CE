...
namespace PCL
{
    public class MyMenuItem : MenuItem
    {
        ...
        private void MyMenuItem_Loaded(object sender, RoutedEventArgs e)
        {
            if (Icon is not null)
            {
                System.Windows.Shapes.Path IconControl = (System.Windows.Shapes.Path)GetTemplateChild("Icon");
                if (IconControl is not null)
                    IconControl.Data = (Geometry)new GeometryConverter().ConvertFromString(Conversions.ToString(Icon));
                // 对父级设置透明度
            } ((ContextMenu)Parent).Opacity = Conversions.ToDouble(Operators.AddObject(Operators.DivideObject(ModBase.Setup.Get("UiLauncherTransparent"), 1000), 0.4d));
        }
        ...
        private void RefreshColor()
        {
            ...
            {
                // 有动画
                ModAnimation.AniStart(new[] { ModAnimation.AaColor(this, BackgroundProperty, BackName, Time), ModAnimation.AaColor(this, ForegroundProperty, ForeName, Time) }, "MyMenuItem Color " + Uuid);
            }
            else
            {
                // 无动画
