...
namespace PCL
{
    public partial class PageDownloadResourcePack
    {
        ...
        // 加载器信息
        public static ModLoader.LoaderTask<ModComp.CompProjectRequest, int> Loader = new ModLoader.LoaderTask<ModComp.CompProjectRequest, int>("CompProject ResourcePack", ModComp.CompProjectsGet, LoaderInput) { ReloadTimeout = 60 * 1000 };
        public static ModComp.CompProjectStorage Storage = new ModComp.CompProjectStorage();
        public static int Page = 0;

        public PageDownloadResourcePack()
        {
            this.Initialized += PageDownloadResourcePack_Inited;
        }
        private void PageDownloadResourcePack_Inited(object sender, EventArgs e)
        {
            this.PageLoaderInit(this.Load, this.PanLoad, this.PanContent, this.PanAlways, Loader, (_) => Load_OnFinish(), PageDownloadResourcePack.LoaderInput);
        ...
        {
            var Request = new ModComp.CompProjectRequest(ModComp.CompType.ResourcePack, Storage, (Page + 1) * PageSize);
            if (ModMain.FrmDownloadResourcePack is not null)
            {
                Request.SearchText = ModMain.FrmDownloadResourcePack.TextSearchName.Text;
                Request.GameVersion = ModMain.FrmDownloadResourcePack.TextSearchVersion.Text == "全部 (也可自行输入)" ? (string)null : ModMain.FrmDownloadResourcePack.TextSearchVersion.Text.Contains(".") || ModMain.FrmDownloadResourcePack.TextSearchVersion.Text.Contains("w") ? ModMain.FrmDownloadResourcePack.TextSearchVersion.Text : (string)null;
                Request.Tag = Conversions.ToString(((dynamic)ModMain.FrmDownloadResourcePack.ComboSearchTag.SelectedItem).Tag);
                Request.Source = (ModComp.CompSourceType)Math.Round(ModBase.Val(((dynamic)ModMain.FrmDownloadResourcePack.ComboSearchSource.SelectedItem).Tag));
            }
            return Request;
        }
        ...
        private void Load_OnFinish()
        {
            try
            {
                ModBase.Log($"[Comp] 开始可视化资源包列表，已储藏 {Storage.Results.Count} 个结果，当前在第 {Page + 1} 页");
                // 列表项
            ...
            {
                ModBase.Log(ex, "可视化资源包列表出错", ModBase.LogLevel.Feedback);
            }
        }
                ...
                case ModBase.LoadState.Failed:
                    {
                        ...
                        {
                            ModBase.Log("[Download] 下载的资源包列表 json 文件损坏，已自动重试", ModBase.LogLevel.Debug);
        ...
        private void ChangePage(int NewPage)
        {
            ...
            ModBase.Log($"[Download] 资源包切换到第 {Page + 1} 页");
            ModBase.RunInThread(() =>
                {
