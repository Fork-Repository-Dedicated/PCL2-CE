...
namespace PCL
{
    public class MyPageRight : AdornerDecorator
    {
        ...
        public MyScrollViewer PanScroll
        {
            get
            {
                ...
                if (res is null)
                    ModBase.Log($"[MyPageRight] 获取到 PanScroll 的值为 null", ModBase.LogLevel.Debug);
        ...
        private static object PanScrollProperty = DependencyProperty.Register("PanScroll", typeof(MyScrollViewer), typeof(MyPageRight));
    ...
    /// <param name="FinishedInvoke">当加载器执行完成，在 UI 线程触发的 UI 初始化事件。</param>
                ...
                RealLoader.PreviewFinish += () =>
        {
            ...
            RealLoader.OnStateChangedUi += (Loader, NewState, OldState) => ModBase.RunInUi(() => PageLoaderState(Loader, NewState, OldState));
            // 隐藏 UI
            ...
            PageLoaderUi.Click += () => { if (RealLoader.State == ModBase.LoadState.Failed) PageLoaderRestart(); }; // 点击重试事件
        }
        // 重试
        public void PageLoaderRestart(object Input = null, bool IsForceRestart = true) // 由外部调用的重试
        {
            ...
            if (PageLoader.GetType().Name.StartsWithF("LoaderTask"))
            {
                PageLoader.Start(((object)PageLoader).StartGetInput(Input, PageLoaderInputInvoke), IsForceRestart: IsForceRestart);
            }
            else
            {
        ...
        public void PageOnEnter()
        {
            ...
            switch (PageState)
            {
                case PageStates.Empty:
                    {
                        if (PageLoader is null || PageLoader.State == ModBase.LoadState.Finished || PageLoader.State == ModBase.LoadState.Waiting || PageLoader.State == ModBase.LoadState.Aborted)
                        {
                        ...
                        else if (PageLoader.State == ModBase.LoadState.Loading)
                        {
                            ...
                            ModAnimation.AniStart(ModAnimation.AaCode(PageOnLoaderWaitFinished, 200), "PageRight PageChange " + PageUuid);
                        }
    ...
    /// </summary>
        private void PageOnEnterAnimationFinished()
        {
            ...
            switch (PageState)
            {
                ...
                default:
                    {
                        throw new Exception("在状态为 " + ModBase.GetStringFromEnum(PageState) + " 时触发了 PageOnEnterAnimationFinished 事件。");
                    }
            }
        }
        /// <summary>
    ...
    /// </summary>
        private void PageOnExitAnimationFinished()
        {
            ...
            switch (PageState)
            {
                ...
                default:
                    {
                        throw new Exception("在状态为 " + ModBase.GetStringFromEnum(PageState) + " 时触发了 PageOnExitAnimationFinished 事件。");
                    }
            }
        }
        /// <summary>
    ...
    /// </summary>
        private void PageOnLoaderWaitFinished()
        {
            ...
            switch (PageState)
            {
                ...
                default:
                    {
                        throw new Exception("在状态为 " + ModBase.GetStringFromEnum(PageState) + " 时触发了 PageOnLoaderWaitFinished 事件。");
                    }
            }
        }
        /// <summary>
    ...
    /// </summary>
        ...
        public void TriggerEnterAnimation(params FrameworkElement[] Elements)
        {
            ...
            foreach (var Element in RealElements)
            {
                foreach (FrameworkElement Control in GetAllAnimControls(Element, true))
                {
                    // 还原被隐藏的卡片的消失动画
                ...
                foreach (FrameworkElement Control in GetAllAnimControls(Element))
                {
                    ...
                    else
                    {
                        ...
                        AniList.Add(ModAnimation.AaOpacity(Control, 1d, 150, Delay, new ModAnimation.AniEaseOutFluent(ModAnimation.AniEasePower.Weak)));
                        AniList.Add(ModAnimation.AaTranslateY(Control, 5d, 250, Delay, new ModAnimation.AniEaseOutFluent()));
            ...
            if (Scroll is not null)
            {
                ...
                AniList.Add(ModAnimation.AaTranslateX(Scroll, -((TranslateTransform)Scroll.RenderTransform).X, 350, 0, new ModAnimation.AniEaseOutFluent()));
            }
            // 结束
        ...
        public void TriggerExitAnimation(params FrameworkElement[] Elements)
        {
            ...
            foreach (var Element in RealElements)
            {
                foreach (FrameworkElement Control in GetAllAnimControls(Element))
                {
                    ...
                    else
                    {
                        ...
                        AniList.Add(ModAnimation.AaOpacity(Control, -1, 90, Delay));
                        AniList.Add(ModAnimation.AaTranslateY(Control, -6, 90, Delay));
                        Delay += 20;
                    }
                }
            }
            ...
            if (Scroll is not null)
            {
                ...
                AniList.Add(ModAnimation.AaTranslateX(Scroll, 10d - ((TranslateTransform)Scroll.RenderTransform).X, 90, 0, new ModAnimation.AniEaseInFluent()));
            }
            // 结束
    ...
    /// 禁用页面切换动画的控件列表。
    /// </summary>
        public List<FrameworkElement> DisabledPageAnimControls = new List<FrameworkElement>();
        /// <summary>
    /// 遍历获取所有需要生成动画的控件。
    /// </summary>
        ...
        private void _GetAllAnimControls(FrameworkElement Element, ref List<FrameworkElement> AllControls, bool IgnoreInvisibility)
        {
            if (!IgnoreInvisibility && Element.Visibility == Visibility.Collapsed)
                return;
            if (Element is MyCard || Element is MyHint || Element is MyExtraTextButton || Element is TextBlock || Element is MyTextButton)
            {
