...
namespace PCL
{
    public class CrashAnalyzer
    {
        ...
        // 1：准备用于分析的 Log 文件
        private List<KeyValuePair<string, string[]>> AnalyzeRawFiles = new List<KeyValuePair<string, string[]>>(); // 暂存的日志文件：文件完整路径 -> 文件内容
                                                                                                                   /// <summary>
        ...
        public void Collect(string VersionPathIndie, IList<string> LatestLog = null)
        {
            ...
            foreach (var LogFile in PossibleLogs)
            {
                try
                {
                    ...
                    if (!Info.Exists)
                        continue;
                    double Time = Math.Abs((Info.LastWriteTime - DateTime.Now).TotalMinutes);
            ...
            foreach (var FilePath in RightLogs)
            {
                try
                {
                    ...
                    {
                        AnalyzeRawFiles.Add(new KeyValuePair<string, string[]>(FilePath, ModBase.ReadFile(FilePath).Split(Constants.vbCrLf.ToCharArray())));
                    }
                    else
                    {
                        AnalyzeRawFiles.Add(new KeyValuePair<string, string[]>(FilePath, ModBase.ReadFile(FilePath, Encoding.UTF8).Split(Constants.vbCrLf.ToCharArray())));
                    }
                }
                catch (Exception ex)
                {
            ...
            {
                string RawOutput = LatestLog.Join(Constants.vbCrLf);
    ...
    /// </summary>
        ...
        Extracted:
            ...
            // 导入其中的日志文件
            foreach (FileInfo TargetFile in new DirectoryInfo(TempFolder + @"Temp\").EnumerateFiles().ToList())
            {
                try
                {
                    if (!TargetFile.Exists || TargetFile.Length == 0L)
                        continue;
                    string Ext = TargetFile.Extension.ToLower();
                    if (Ext == ".log" || Ext == ".txt")
                    {
                        if (TargetFile.Name.StartsWithF("crash-"))
                        {
                            AnalyzeRawFiles.Add(new KeyValuePair<string, string[]>(TargetFile.FullName, ModBase.ReadFile(TargetFile.FullName).Split(Constants.vbCrLf.ToCharArray())));
                        }
                        else
                        {
                            AnalyzeRawFiles.Add(new KeyValuePair<string, string[]>(TargetFile.FullName, ModBase.ReadFile(TargetFile.FullName, Encoding.UTF8).Split(Constants.vbCrLf.ToCharArray())));
                        }
                    }
                    else
                    {
        ...
        private KeyValuePair<string, string[]>? DirectFile = default; // 在弹窗中选择直接打开的文件
                                                                      /// <summary>
    ...
    /// </summary>
        public int Prepare()
        {
            ...
            // 对日志文件进行分类
            DirectFile = default;
            var TotalFiles = new List<KeyValuePair<AnalyzeFileType, KeyValuePair<string, string[]>>>();
            foreach (var LogFile in AnalyzeRawFiles)
            {
                string MatchName = ModBase.GetFileNameFromPath(LogFile.Key).ToLower();
                ...
                else if (MatchName == "latest.log" || MatchName == "latest log.txt" || MatchName == "debug.log" || MatchName == "debug log.txt" || MatchName == "游戏崩溃前的输出.txt" || MatchName == "rawoutput.log")
                {
                    TargetType = AnalyzeFileType.MinecraftLog;
                    if (DirectFile is null)
                        DirectFile = LogFile;
                }
                else if (MatchName == "启动器日志.txt" || MatchName == "PCL2 启动器日志.txt" || MatchName == "PCL 启动器日志.txt" || MatchName == "log1.txt" || MatchName == "log-ce1.log")
                {
                ...
                else if (MatchName.EndsWithF(".log", true) || MatchName.EndsWithF(".txt", true))
                {
                    TargetType = AnalyzeFileType.ExtraLog;
                }
                else
                ...
                {
                    TotalFiles.Add(new KeyValuePair<AnalyzeFileType, KeyValuePair<string, string[]>>(TargetType, LogFile));
                    ModBase.Log("[Crash] " + MatchName + " 分类为 " + ModBase.GetStringFromEnum(TargetType));
                }
            }

            // 将分类后的文件分别写入
            foreach (var SelectType in new[] { AnalyzeFileType.MinecraftLog, AnalyzeFileType.HsErr, AnalyzeFileType.ExtraLog, AnalyzeFileType.CrashReport })
            {
                        ...
                        case AnalyzeFileType.MinecraftLog:
                            {
                                ...
                                // 选择一份最佳的来自启动器的游戏日志
                                foreach (string FileName in new[] { "rawoutput.log", "启动器日志.txt", "log1.txt", "log-ce1.log", "游戏崩溃前的输出.txt", "PCL2 启动器日志.txt", "PCL 启动器日志.txt" })
                                {
                                    if (FileNameDict.ContainsKey(FileName))
                                    {
                                        var CurrentLog = FileNameDict[FileName];
                                        // 截取 “以下为游戏输出的最后一段内容” 后的内容
                                        bool HasLauncherMark = false;
                                ...
                                foreach (string FileName in new[] { "latest.log", "latest log.txt", "debug.log", "debug log.txt" })
                                {
                                    if (FileNameDict.ContainsKey(FileName))
                                    {
                                        var CurrentLog = FileNameDict[FileName];
                                ...
                                // 查找 Debug Log
                                foreach (string FileName in new[] { "debug.log", "debug log.txt" })
                                {
                                    if (FileNameDict.ContainsKey(FileName))
                                    {
                                        var CurrentLog = FileNameDict[FileName];
                        ...
                        case AnalyzeFileType.ExtraLog:
                            {
                                ...
                                {
                                    OutputFiles.Add(SelectedFile.Key);
                                    ModBase.Log("[Crash] 输出报告：" + SelectedFile.Key + "，作为额外日志");
                                }
            ...
            // 获取种类数
            int ResultCount = (LogMc is null ? 0 : 1) + (LogHs is null ? 0 : 1) + (LogCrash is null ? 0 : 1);
            if (ResultCount == 0)
            {
            ...
            {
                ModBase.Log(("[Crash] 步骤 2：准备日志文本完成，找到" + (LogMc is null ? "" : "游戏日志、") + (LogMcDebug is null ? "" : "游戏 Debug 日志、") + (LogHs is null ? "" : "虚拟机日志、") + (LogCrash is null ? "" : "崩溃日志、")).TrimEnd('、') + "用作分析");
            }
            return ResultCount;
    ...
    /// </summary>
        ...
        // 3：根据文本分析崩溃原因
        private string LogMc = null;
        private string LogMcDebug = null;
        private string LogHs = null;
        private string LogCrash = null;
        private string LogAll;
        // 可能导致崩溃的原因与附加信息
        private Dictionary<CrashReason, List<string>> CrashReasons = new Dictionary<CrashReason, List<string>>();
        /// <summary>
    /// 导致崩溃的原因枚举。
    /// </summary>
    ...
    /// </summary>
        public void Analyze(ModMinecraft.McVersion Version = null)
        {
            ...
            // 2. 堆栈分析
            if (LogAll.Contains("orge") || LogAll.Contains("abric") || LogAll.Contains("uilt") || LogAll.Contains("iteloader"))
            {
                ...
                if (LogHs is not null)
                {
                    ...
                    string StackLogs = LogHs.Between("T H R E A D", "Registers:");
        ...
        Done:
            ...
            else
            {
                ...
                foreach (var Reason in CrashReasons)
                    ModBase.Log("[Crash]  - " + ModBase.GetStringFromEnum(Reason.Key) + (Reason.Value.Any() ? "（" + Reason.Value.Join("；") + "）" : ""));
            }
        }
        /// <summary>
    /// 增加一个可能的崩溃原因。
    /// </summary>
        private void AppendReason(CrashReason Reason, ICollection<string> Additional = null)
        {
            ...
            ModBase.Log("[Crash] 可能的崩溃原因：" + ModBase.GetStringFromEnum(Reason) + (Additional is not null && Additional.Any() ? "（" + Additional.Join("；") + "）" : ""));
        }
        ...
        private void AnalyzeCrit1()
        {
            ...
            if (LogMc is not null)
            {
                if (LogMc.Contains("Found multiple arguments for option fml.forgeVersion, but you asked for only one"))
                    AppendReason(CrashReason.版本Json中存在多个Forge);
                ...
                if (LogMc.Contains("java.lang.NoSuchMethodError: 'void net.minecraft.client.renderer.texture.SpriteContents.<init>"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                if (LogMc.Contains("java.lang.NoSuchMethodError: 'java.lang.String com.mojang.blaze3d.systems.RenderSystem.getBackendDescription"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                if (LogMc.Contains("java.lang.NoSuchMethodError: 'void net.minecraft.client.renderer.block.model.BakedQuad.<init>"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                if (LogMc.Contains("java.lang.NoSuchMethodError: 'void net.minecraftforge.client.gui.overlay.ForgeGui.renderSelectedItemName"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                if (LogMc.Contains("java.lang.NoSuchMethodError: 'void net.minecraft.server.level.DistanceManager"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                if (LogMc.Contains("java.lang.NoSuchMethodError: 'net.minecraft.network.chat.FormattedText net.minecraft.client.gui.Font.ellipsize"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                if (LogMc.Contains("Open J9 is not supported") || LogMc.Contains("OpenJ9 is incompatible") || LogMc.Contains(".J9VMInternals."))
                ...
                if (LogMc.Contains("because module java.base does not export"))
                    AppendReason(CrashReason.Java版本过高);
                if (LogMc.Contains("java.lang.ClassNotFoundException: jdk.nashorn.api.scripting.NashornScriptEngineFactory"))
                    AppendReason(CrashReason.Java版本过高);
                ...
                if (LogMc.Contains("java.lang.RuntimeException: Shaders Mod detected. Please remove it, OptiFine has built-in support for shaders."))
                ...
                if (LogMc.Contains("signer information does not match signer information of other classes in the same package"))
                    AppendReason(CrashReason.文件或内容校验失败, (LogMc.RegexSeek("(?<=class \")[^']+(?=\"'s signer information)") ?? "").TrimEnd(Conversions.ToChar(Constants.vbCrLf)));
                if (LogMc.Contains("Maybe try a lower resolution resourcepack?"))
                    AppendReason(CrashReason.材质过大或显卡配置不足);
                if (LogMc.Contains("java.lang.NoSuchMethodError: net.minecraft.world.server.ChunkManager$ProxyTicketManager.shouldForceTicks(J)Z") && LogMc.Contains("OptiFine"))
                ...
                if (LogMc.Contains("Invalid paths argument, contained no existing paths") && LogMc.Contains(@"libraries\net\minecraftforge\fmlcore"))
                    AppendReason(CrashReason.Forge安装不完整);
                if (LogMc.Contains("Invalid module name: '' is not a Java identifier"))
                    AppendReason(CrashReason.Mod名称包含特殊字符);
                if (LogMc.Contains("has been compiled by a more recent version of the Java Runtime (class file version 55.0), this version of the Java Runtime only recognizes class file versions up to"))
                    AppendReason(CrashReason.Mod需要Java11);
                if (LogMc.Contains("java.lang.RuntimeException: java.lang.NoSuchMethodException: no such method: sun.misc.Unsafe.defineAnonymousClass(Class,byte[],Object[])Class/invokeVirtual"))
                    AppendReason(CrashReason.Mod需要Java11);
                if (LogMc.Contains("java.lang.IllegalArgumentException: The requested compatibility level JAVA_11 could not be set. Level is not supported by the active JRE or ASM version"))
                    AppendReason(CrashReason.Mod需要Java11);
                ...
                if (LogMc.Contains("Caught exception from "))
                    AppendReason(CrashReason.确定Mod导致游戏崩溃, TryAnalyzeModName(LogMc.RegexSeek(@"(?<=Caught exception from )[^\n]+?")?.TrimEnd((Constants.vbCrLf + " ").ToCharArray())));
                // Mod 重复 / 前置问题
                if (LogMc.Contains("DuplicateModsFoundException"))
                    AppendReason(CrashReason.Mod重复安装, LogMc.RegexSearch(@"(?<=\n\t[\w]+ : [A-Z]{1}:[^\n]+(/|\\))[^/\\\n]+?.jar", System.Text.RegularExpressions.RegexOptions.IgnoreCase));
                if (LogMc.Contains("Found a duplicate mod"))
                    AppendReason(CrashReason.Mod重复安装, (LogMc.RegexSeek(@"Found a duplicate mod[^\n]+") ?? "").RegexSearch(@"[^\\/]+.jar", System.Text.RegularExpressions.RegexOptions.IgnoreCase));
                if (LogMc.Contains("Found duplicate mods"))
                    AppendReason(CrashReason.Mod重复安装, ModBase.RegexSearch(LogMc, @"(?<=Mod ID: ')\w+?(?=' from mod files:)").Distinct().ToList());
                if (LogMc.Contains("ModResolutionException: Duplicate"))
                    AppendReason(CrashReason.Mod重复安装, (LogMc.RegexSeek(@"ModResolutionException: Duplicate[^\n]+") ?? "").RegexSearch(@"[^\\/]+.jar", System.Text.RegularExpressions.RegexOptions.IgnoreCase));
                if (LogMc.Contains("Incompatible mods found!")) // #5006
                {
                    AppendReason(CrashReason.Mod互不兼容, LogMc.RegexSeek(@"(?<=Incompatible mods found![\s\S]+: )[\s\S]+?(?=\tat )") ?? "");
                }
                if (LogMc.Contains("Missing or unsupported mandatory dependencies:"))
                {
                    AppendReason(CrashReason.Mod缺少前置或MC版本错误, ModBase.RegexSearch(LogMc, @"(?<=Missing or unsupported mandatory dependencies:)([\n\r]+\t(.*))+", System.Text.RegularExpressions.RegexOptions.IgnoreCase).Select(s => s.Trim((Constants.vbCrLf + Constants.vbTab + " ").ToCharArray())).Distinct().ToList());
                }
            }
            ...
            if (LogCrash is not null)
            {
                ...
                if (LogCrash.Contains("has mods that were not found") && LogCrash.RegexCheck(@"The Mod File [^\n]+optifine\\OptiFine[^\n]+ has mods that were not found"))
                    AppendReason(CrashReason.OptiFine与Forge不兼容);
                // Mod 导致的崩溃
                if (LogCrash.Contains("-- MOD "))
                {
                    string LogCrashMod = LogCrash.Between("-- MOD ", "Failure message:");
                    if (LogCrashMod.ContainsF(".jar", true))
                    {
                        AppendReason(CrashReason.确定Mod导致游戏崩溃, (LogCrashMod.RegexSeek("(?<=Mod File: ).+") ?? "").TrimEnd((Constants.vbCrLf + " ").ToCharArray()));
                    }
                    else
                    {
                        AppendReason(CrashReason.Mod加载器报错, (LogCrash.RegexSeek(@"(?<=Failure message: )[\w\W]+?(?=\tMod)") ?? "").Replace(Constants.vbTab, " ").TrimEnd((Constants.vbCrLf + " ").ToCharArray()));
                    }
                }
                if (LogCrash.Contains("Multiple entries with same key: "))
                    AppendReason(CrashReason.确定Mod导致游戏崩溃, TryAnalyzeModName((LogCrash.RegexSeek("(?<=Multiple entries with same key: )[^=]+") ?? "").TrimEnd((Constants.vbCrLf + " ").ToCharArray())));
                if (LogCrash.Contains("LoaderExceptionModCrash: Caught exception from "))
                    AppendReason(CrashReason.确定Mod导致游戏崩溃, TryAnalyzeModName((LogCrash.RegexSeek(@"(?<=LoaderExceptionModCrash: Caught exception from )[^\n]+") ?? "").TrimEnd((Constants.vbCrLf + " ").ToCharArray())));
                if (LogCrash.Contains("Failed loading config file "))
                    AppendReason(CrashReason.Mod配置文件导致游戏崩溃, new[] { TryAnalyzeModName((ModBase.RegexSeek(LogCrash, @"(?<=Failed loading config file .+ for modid )[^\n]+") ?? "").TrimEnd(Conversions.ToChar(Constants.vbCrLf))).First(), (LogCrash.RegexSeek("(?<=Failed loading config file ).+(?= of type)") ?? "").TrimEnd(Conversions.ToChar(Constants.vbCrLf)) });
            }
    ...
    /// </summary>
        private void AnalyzeCrit2()
        {
            ...
            {
                bool IsMixin = LogText.Contains("Mixin prepare failed ") || LogText.Contains("Mixin apply failed ") || LogText.Contains("MixinApplyError") || LogText.Contains("MixinTransformerError") || LogText.Contains("mixin.injection.throwables.") || LogText.Contains(".json] FAILED during )");
                if (!IsMixin)
                    return false;
                // Mod 名称匹配
                string ModName = LogText.RegexSeek(@"(?<=from mod )[^.\/ ]+(?=\] from)");
                if (ModName is null)
                    ModName = LogText.RegexSeek(@"(?<=for mod )[^.\/ ]+(?= failed)");
                if (ModName is not null)
                {
                    AppendReason(CrashReason.ModMixin失败, TryAnalyzeModName(ModName.TrimEnd((Constants.vbCrLf + " ").ToCharArray())));
                    return true;
                }
                // JSON 名称匹配
                foreach (var JsonName in LogText.RegexSearch(@"(?<=^[^\t]+[ \[{(]{1})[^ \[{(]+\.[^ ]+(?=\.json)", System.Text.RegularExpressions.RegexOptions.Multiline))
                {
            ...
            {
                // Mixin 崩溃
                bool IsMixin = MixinAnalyze(LogMc);
                // 常规信息
                if (LogMc.Contains("An exception was thrown, the game will display an error screen and halt."))
                    AppendReason(CrashReason.Forge报错, LogMc.RegexSeek(@"(?<=the game will display an error screen and halt.[\n\r]+[^\n]+?Exception: )[\s\S]+?(?=\n\tat)")?.Trim(Conversions.ToChar(Constants.vbCrLf)));
                if (LogMc.Contains("A potential solution has been determined:"))
                    AppendReason(CrashReason.Fabric报错并给出解决方案, (LogMc.RegexSeek(@"(?<=A potential solution has been determined:\n)((\t)+ - [^\n]+\n)+") ?? "").RegexSearch(@"(?<=(\t)+)[^\n]+").Join(Constants.vbLf));
                if (LogMc.Contains("A potential solution has been determined, this may resolve your problem:"))
                    AppendReason(CrashReason.Fabric报错并给出解决方案, (LogMc.RegexSeek(@"(?<=A potential solution has been determined, this may resolve your problem:\n)((\t)+ - [^\n]+\n)+") ?? "").RegexSearch(@"(?<=(\t)+)[^\n]+").Join(Constants.vbLf));
                if (LogMc.Contains("确定了一种可能的解决方法，这样做可能会解决你的问题："))
                    AppendReason(CrashReason.Fabric报错并给出解决方案, (LogMc.RegexSeek(@"(?<=确定了一种可能的解决方法，这样做可能会解决你的问题：\n)((\t)+ - [^\n]+\n)+") ?? "").RegexSearch(@"(?<=(\t)+)[^\n]+").Join(Constants.vbLf));
                if (!IsMixin && LogMc.Contains("due to errors, provided by ")) // 在 #3104 的情况下，这一句导致 OptiFabric 的 Mixin 失败错判为 Fabric Loader 加载失败
                {
                    AppendReason(CrashReason.确定Mod导致游戏崩溃, TryAnalyzeModName((LogMc.RegexSeek("(?<=due to errors, provided by ')[^']+") ?? "").TrimEnd((Constants.vbCrLf + " ").ToCharArray())));
                }
            }
            ...
            if (LogCrash is not null)
            {
                ...
                {
                    string SuspectsRaw = LogCrash.Between("Suspected Mod", "Stacktrace");
    ...
    /// </summary>
        private void AnalyzeCrit3()
        {
            ...
            {
                // 极短的程序输出
                if (!(LogMc.Contains("at net.") || LogMc.Contains("INFO]")) && LogHs is null && LogCrash is null && LogMc.Length < 100)
                {
                ...
                if (LogMc.Contains("Failed to create mod instance."))
                    AppendReason(CrashReason.Mod初始化失败, TryAnalyzeModName((LogMc.RegexSeek("(?<=Failed to create mod instance. ModID: )[^,]+") ?? LogMc.RegexSeek(@"(?<=Failed to create mod instance. ModId )[^\n]+(?= for )") ?? "").TrimEnd(Conversions.ToChar(Constants.vbCrLf))));
            ...
            if (LogCrash is not null)
            {
                if (LogCrash.Contains(Constants.vbTab + "Block location: World: "))
                    AppendReason(CrashReason.特定方块导致崩溃, (LogCrash.RegexSeek(@"(?<=\tBlock: Block\{)[^\}]+") ?? "") + " " + (LogCrash.RegexSeek(@"(?<=\tBlock location: World: )\([^\)]+\)") ?? ""));
                if (LogCrash.Contains(Constants.vbTab + "Entity's Exact location: "))
                    AppendReason(CrashReason.特定实体导致崩溃, (LogCrash.RegexSeek(@"(?<=\tEntity Type: )[^\n]+(?= \()") ?? "") + " (" + (LogCrash.RegexSeek(@"(?<=\tEntity's Exact location: )[^\n]+") ?? "").TrimEnd(Constants.vbCrLf.ToCharArray()) + ")");
            }
    ...
    /// </summary>
        private List<string> AnalyzeStackKeyword(string ErrorStack)
        {
            ...
            StackSearchResults.AddRange(ErrorStack.RegexSearch(@"(?<=\n[^{]+)[a-zA-Z_]+\w+\.[a-zA-Z_]+[\w\.]+(?=\.[\w\.$]+\.)"));
            StackSearchResults.AddRange(ModBase.RegexSearch(ErrorStack, @"(?<=at [^(]+?\.\w+\$\w+\$)[\w\$]+?(?=\$\w+\()").Select(s => s.Replace("$", "."))); // Mixin 堆栈：xxx.xxx.xxxx$xxxx$xxx
            StackSearchResults = StackSearchResults.Distinct().ToList();

            // 检查堆栈开头
            var PossibleStacks = new List<string>();
            foreach (string Stack in StackSearchResults)
            {
                // If Not Stack.Contains(".") Then Continue For
                foreach (var IgnoreStack in new[] { "java", "sun", "javax", "jdk", "oolloo", "org.lwjgl", "com.sun", "net.minecraftforge", "paulscode.sound", "com.mojang", "net.minecraft", "cpw.mods", "com.google", "org.apache", "org.spongepowered", "net.fabricmc", "com.mumfrey", "com.electronwill.nightconfig", "it.unimi.dsi", "MojangTricksIntelDriversForPerformance_javaw" })
                {
            ...
            foreach (string Stack in PossibleStacks)
    ...
    /// </summary>
        private List<string> AnalyzeModName(List<string> Keywords)
        {
            ...
            {
                string Details = LogCrash.Replace("A detailed walkthrough of the error", "¨");
                bool IsFabricDetail = Details.Contains("Fabric Mods"); // 是否为 Fabric 信息格式
                if (IsFabricDetail)
                {
                ...
                {
                    if (Line.ContainsF(".jar", true) && Line.Length - Line.Replace(".jar", "").Length == 4 || IsFabricDetail && Line.StartsWithF(Constants.vbTab + Constants.vbTab) && !Line.RegexCheck(@"\t\tfabric[\w-]*: Fabric")) // 只有一个 .jar
                ...
                {
                    foreach (string ModString in ModNameLines)
                    {
                        string RealModString = ModString.ToLower().Replace("_", "");
                        if (!RealModString.Contains(KeyWord.ToLower().Replace("_", "")))
                            continue;
                        if (RealModString.Contains("minecraft.jar") || RealModString.Contains(" forge-") || RealModString.Contains(" mixin-"))
                            continue;
                ...
                foreach (string ModLine in HintLines)
                ...
                foreach (string Line in HintLines)
                {
                    ...
                    {
                        Name = Line.RegexSeek(@"(?<=\()[^\t]+.jar(?=\))|(?<=(\t\t)|(\| ))[^\t\|]+.jar", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                    }
                    if (Name is not null)
            ...
            if (LogMcDebug is not null)
            {
                ...
                var ModNameLines = LogMcDebug.RegexSearch("(?<=valid mod file ).*", System.Text.RegularExpressions.RegexOptions.Multiline);
            ...
            else
            {
                ...
                foreach (string ModFileName in ModFileNames)
    ...
    /// </summary>
        public void Output(bool IsHandAnalyze, List<string> ExtraFiles = null)
        {
            ...
            // 弹窗选择：查看日志
            switch (ModMain.MyMsgBox(GetAnalyzeResult(IsHandAnalyze), IsHandAnalyze ? "错误报告分析结果" : "Minecraft 出现错误", "确定", IsHandAnalyze || DirectFile is null ? "" : "查看日志", IsHandAnalyze ? "" : "导出错误报告", Button2Action: IsHandAnalyze || DirectFile is null ? (var)null : new Action(() => { if (File.Exists(DirectFile.Value.Key)) { ModBase.ShellOnly(DirectFile.Value.Key); } else { string FilePath = ModBase.PathTemp + "Crash.txt"; ModBase.WriteFile(FilePath, DirectFile.Value.Value.Join(Constants.vbCrLf)); ModBase.ShellOnly(FilePath); } })))
            {
                case 3:
                    {
                        // 弹窗选择：导出错误报告
                        ...
                        {
                            // 获取文件路径
                            ModBase.RunInUiWait(() => FileAddress = ModBase.SelectSaveFile("选择保存位置", "错误报告-" + DateTime.Now.ToString("G").Replace("/", "-").Replace(":", ".").Replace(" ", "_") + ".zip", "Minecraft 错误报告(*.zip)|*.zip"));
                            ...
                            {
                                string FileName = ModBase.GetFileNameFromPath(OutputFile);
                                ...
                                if (File.Exists(OutputFile))
                                {
                                    if (FileEncoding is null)
                                        FileEncoding = ModBase.GetEncoding(ModBase.ReadFileBytes(OutputFile));
                                    ModBase.WriteFile(TempFolder + @"Report\" + FileName, ModSecret.SecretFilter(ModBase.ReadFile(OutputFile, FileEncoding), Conversions.ToChar(FileName == "启动脚本.bat" ? "F" : "*")), Encoding: FileEncoding);
                                    ModBase.Log($"[Crash] 导出文件：{FileName}，编码：{FileEncoding.HeaderName}");
                                }
                            }
                            // 导出报告
                            System.IO.Compression.ZipFile.CreateFromDirectory(TempFolder + @"Report\", FileAddress);
        ...
        private string GetAnalyzeResult(bool IsHandAnalyze)
        {
            ...
            foreach (var Reason in CrashReasons)
            {
                ...
                switch (Reason.Key)
                {
                    ...
                    default:
                        {
                            Results.Add("PCL 获取到了没有详细信息的错误原因（" + ((int)CrashReasons.First().Key).ToString() + @"），请向 PCL 作者提交反馈以获取详情。\h");
                            break;
                        }
                }
            }

            return Results.Join(@"\n\n此外，").Replace(@"\n", Constants.vbCrLf).Replace(@"\h", "").Replace(@"\e", IsHandAnalyze ? "" : Constants.vbCrLf + "你可以查看错误报告了解错误具体是如何发生的。").Replace(Constants.vbCrLf, Constants.vbCr).Replace(Constants.vbLf, Constants.vbCr).Replace(Constants.vbCr, Constants.vbCrLf).Trim(Constants.vbCrLf.ToCharArray()) + (!Results.Any(r => r.EndsWithF(@"\h")) || IsHandAnalyze ? "" : Constants.vbCrLf + "如果要寻求帮助，请把错误报告文件发给对方，而不是发送这个窗口的照片或者截图。" + (PageSetupSystem.IsLauncherNewest() ?? true ? "" : Constants.vbCrLf + Constants.vbCrLf + "此外，你正在使用老版本 PCL，更新 PCL 或许也能解决这个问题。" + Constants.vbCrLf + "你可以点击 设置 → 启动器 → 检查更新 来更新 PCL。"));
        }
